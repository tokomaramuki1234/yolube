...
| `/` | メインページ | ✅ 実装済 |
| `/ke` | Ke.イベントページ | ✅ 実装済 |
| `/training` | 企業研修ページ（オリジナル） | ✅ 実装済（Phase 2スクロールアニメーション適用済） |
| `/training01` | 企業研修ページ（ABテスト用） | ✅ 実装済 |
| `/NEWS` | 新着情報ページ | ✅ 実装済 |
| `/admin` | 管理画面（認証あり） | ✅ 実装済 |
| `/hero-demo` | ヒーローアニメーションデモ | ✅ 実装済（5パターン比較用） |
| `/h2-demo` | H2見出しアニメーションデモ | ✅ 実装済（6パターン比較用） |
...
5. **WebGL風アニメーションシステム**
   ...
   - **Phase 1 - ヒーローセクション**: パターン4「分裂→集合」テキストアニメーション
     - 文字が画面外のランダムな位置から集まってくる動的演出
     - サブタイトルはブラーインエフェクトで滑らかに表示
   - **Phase 2 - ページ全体** ✅ 実装完了（2025年10月29日）
     - H2/H3/H4見出し: 文字単位スケール拡大アニメーション（パターン4）
     - P段落/Liリスト: ブロック単位フェードインアニメーション
     - Intersection Observer APIによるスクロール連動
     - 20%閾値で自然な発火タイミング
   - **アニメーションデモページ**:
     - `/hero-demo`: ヒーローセクション 5パターン比較
     - `/h2-demo`: H2見出し 6パターン比較
   - 実装日: Phase 1 (2025年10月28日) / Phase 2 (2025年10月29日)
