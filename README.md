...
### 2025年10月12日: マージコンフリクトによるVercelビルドエラー

#### 📋 問題の経緯

**目的**: NEWS管理システムにX (Twitter) 自動投稿機能を追加

**発生した問題**: 
1. 管理画面 (`/admin`) にログインできない
   - パスワード `yolube2025` が機能せず、`default_password` でしかログインできない
2. Vercelデプロイが連続してエラーで失敗
   - コミット `beff1f6`, `36921d6`, `e9a73de` がすべてビルドエラー

#### 🔍 根本原因

**Gitマージコンフリクトマーカーが複数ファイルに残存**

過去のマージ操作（コミット `c571c0f`）でコンフリクト解決が不完全だったため、以下のファイルに `<<<<<<< HEAD`, `=======`, `>>>>>>>` マーカーが残っていた：

1. `src/contexts/AuthContext.jsx` - パスワード定義部分
2. `src/pages/admin/Admin.jsx` - インポート文、関数定義
3. `src/components/admin/NewsEditor.jsx` - コンポーネント全体
4. `src/components/admin/NewsEditor.css` - スタイル定義
5. `README.md` - ドキュメント全体

これらがReactビルド時に構文エラーとなり、Vercelデプロイが失敗していた。

#### ✅ 解決方法

##### 1. **AuthContext.jsx の修正** (コミット `beff1f6`)
- 環境変数フォールバックを削除
- パスワードを直接ハードコード化
```javascript
// 修正前
...
##### 2. **Admin.jsx のマージコンフリクト解決** (コミット `36921d6`)
- 6-9行目: インポート文のマージマーカー削除
- 25-28行目: 重複した `NEWS_API_URL` 定義を1つに統合
- 196-210行目: ナビゲーションボタンのマージマーカー削除
- 225-233行目: ページタイトルのマージマーカー削除
- 258-277行目: NEWSタブのレンダリング部分のマージマーカー削除

##### 3. **NewsEditor.jsx と README.md の修正** (コミット `e9a73de`)
- NewsEditor.jsx: コミット `13ef452` の正しいバージョンを復元
- README.md: コミット `74e2628` のクリーンなバージョンを適用

##### 4. **NewsEditor.css の修正** (コミット `e977046`)
- コミット `13ef452` の正しいバージョンを復元
- すべてのマージマーカーを削除

#### 📊 解決結果

- ✅ Vercelビルド成功（デプロイメント `EwHpEG4Ky`）
- ✅ 管理画面ログイン成功（パスワード `yolube2025`）
- ✅ ダッシュボード、NEWS管理タブ正常表示

#### 💡 学んだ教訓

##### ❌ やってはいけないこと
1. **ローカルビルドテストをせずにプッシュする**
   - マージコンフリクトがあるとビルドが失敗する
   - `npm run build` で事前確認が必要
   
2. **マージコンフリクトマーカーを見落とす**
   - `<<<<<<< HEAD`, `=======`, `>>>>>>>` は必ず削除
   - 複数ファイルに分散している可能性がある
   
3. **エラーが出たら根本原因を特定せずに進める**
   - ビルドログを必ず確認
   - 構文エラーの行番号とファイル名を特定
...
- [ ] **X (Twitter) 投稿機能のテスト**
  - 管理画面でNEWS記事作成
  - 「X (Twitter) に投稿する」チェックボックスをON
  - 保存後、X投稿成功メッセージを確認
  - 実際にXアカウントで投稿を確認

##### 優先度: 中
- [ ] **n8n webhook連携の検討**
  - X API直接呼び出しからn8n経由に変更する可能性
  - 現在のOAuth 1.0a実装の安定性を評価
...
*最終更新: 2025年10月12日*
*システムバージョン: v4.1 (X投稿機能実装、マージコンフリクト解決)*